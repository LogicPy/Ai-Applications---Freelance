...
class ChatSession(db.Model):
    ...
    context = db.Column(db.Text(), nullable=True, default=None)  # Changed from False to True
...
def chat():
    try:
        ...
        message = data.get('message')  # Changed from 'prompt' to 'message'
        ai_framework = data.get('ai_framework')  # If needed
        ...
        chat_session.context += f"User: {message}\n"  # Ensure 'context' exists
...
def get_ollama_response(message, chat_id):
    ...
    try:
        ...
        logging.debug(f"Ollama API response: {response.text}")
        ...
        for resp in responses:
            try:
                ...
                if 'message' in resp_json and 'content' in resp_json['message']:
                    ai_message = resp_json['message']['content']
                    ai_messages.append(ai_message.strip())
            except json.JSONDecodeError:
                logging.warning(f"Skipping invalid JSON response: {resp}")
                continue
